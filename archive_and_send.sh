...
pushd trace_results
for d in $(ls | grep UNET_)
do
    echo $d
    tar cvzf ../archive/${d}.tar.gz $d
    [ $? != 0 ] && exit 1
    scp ../archive/${d}.tar.gz lhovon@discslab-server1:/data/lhovon/traces/UNET_original_1w_2
done
popd

pushd trace_results
for d in $(ls | grep -E 'DLRM_.*_32ksteps')
do
    echo $d
    tar cvzf ../archive/${d}.tar.gz $d
    [ $? != 0 ] && exit 1
    scp ../archive/${d}.tar.gz lhovon@discslab-server1:/data/lhovon/traces/DLRM_32ksteps_2
done
popd
