...
do
		...
		do
				...
				folder="${GPU_NUM}gpu_${DATA_SIZE}gb"
				mkdir -p $output_dir/$folder 


				echo "BEGGINING TRACE OF $GPU_NUM gpus, DATASET SIZE OF: $DATA_SIZE gb, AND CONTAINER SIZE OF $MEMORY_CONSTRAINT gb"
				...
				bpftrace traces/trace_bio.bt -o ${output_dir}/${folder}/trace_bio.out &
				trace_bio_pid=$!

				bpftrace traces/trace_read.bt -o ${output_dir}/${folder}/trace_read.out &
				trace_read_pid=$!

				bpftrace traces/trace_write.bt -o ${output_dir}/${folder}/trace_write.out &
				trace_write_pid=$!

				bpftrace traces/trace_create_del.bt -o ${output_dir}/${folder}/trace_create_del.out &
				trace_create_del_pid=$!

				bpftrace traces/trace_openat.bt -o ${output_dir}/${folder}/trace_openat.out &
				trace_openat_pid=$!

				bpftrace traces/trace_close.bt -o ${output_dir}/${folder}/trace_close.out &
				trace_close_pid=$!

				bpftrace traces/trace_mmap.bt -o ${output_dir}/${folder}/trace_mmap.out &
				trace_mmap_pid=$!

				# Start time alignment trace
				bpftrace traces/trace_time_align.bt -o ${output_dir}/${folder}/trace_time_align.out &
				trace_time_align_pid=$!

				# Start the CPU and GPU traces
				mpstat 1 > ${output_dir}/${folder}/cpu.out &
				trace_cpu_pid=$!

				#TODO: Explore using Nsight for GPU tracing
				nvidia-smi pmon -s um -o DT -f ${output_dir}/${folder}/gpu.out &		
				trace_gpu_pid=$!

				echo "Starting training"
				# Start training within the tmux session. 
				tmux send-keys -t training "sudo ${workload_dir}/start_training.sh -g ${GPU_NUM} -r 1 -m ${MEMORY_CONSTRAINT}" C-m
				...
				ps aux -T | grep python > ${output_dir}/${folder}/pids.out
				...
				cp ${workload_dir}/outputs/bert.log $output_dir/$folder

				# Archive the traces
				output_parent_dir="$(dirname "$output_dir/$folder")"
