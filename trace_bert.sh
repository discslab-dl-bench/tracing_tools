...
usage () {
	echo "Usage: $0 <workload_dir> <output_dir> <num_gpus> <amt_memory> <dataset_size> (<experiment_name>)"
	exit 1
}
...
tmux send-keys -t training "sudo ${workload_dir}/start_training.sh -g ${num_gpus} -r 1 -m ${amt_memory} -d ${data_dir}" C-m
...
cp ${workload_dir}/outputs/bert.log $output_dir
